...
// TODO: Is there a way to automate these output type based on the format declare in Fabric.cs ?
...
)
{
	...
	if (surfaceData.materialId == MATERIALID_LIT_STANDARD)
	{
		...
		outGBuffer2 = float4(octTangentWS * 0.5 + 0.5, surfaceData.anisotropy, 1);
	}
...
#if !defined(FABRIC_BSDF)
	#ifndef _FABRIC_SILK
		#define FABRIC_BSDF BSDF_Silk
	#else
		#define FABRIC_BSDF BSDF_CottonWool
	#endif
#endif
	...
	out float3 specularLighting)
{
	...
	{
		FABRIC_BSDF(V, L, positionWS, preLightData, bsdfData, diffuseLighting, specularLighting);
	...
	int sampleCount = 128)
{
	...
	for (int i = 0; i < sampleCount; ++i)
	{
		...
		if (NdotL > 0)
		{
			...
			FABRIC_BSDF(V, L, positionWS, preLightData, bsdfData, lightDiff, lightSpec);
	...
	uint sampleCount = 512)
{
	...
	for (uint i = 0; i < sampleCount; ++i)
	{
		...
		if (illuminance > 0.0)
		{
			FABRIC_BSDF(V, L, positionWS, preLightData, bsdfData, localDiffuseLighting, localSpecularLighting);
